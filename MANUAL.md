...
Let's write a very simple plugin.
First, make sure you completed all steps from the [quickstart](#how-to-generate-code). We'll add two new dependencies:
...
Create a new file `src/protoc-gen-hello.ts`:
...
We can run the plugin with `tsx` to see it working. It will print its name and version:

```shellsession
npx tsx src/protoc-gen-hello.ts --version
```

To feed a schema to the plugin, add it to your `buf.gen.yaml` file:
...
You would typically loop through files of the schema, and generate a file for each Protobuf file. The property
...
To generate a file, call the method `Schema.generateFile`. It takes a file names as an argument. By convention, you
...
We recommend that you add a preamble to every generated file. It provides helpful information about the plugin, and
...
> [!NOTE]
>
> If you don't print anything to a generated file, it will not be generated.
...
To print documentation comments, take a look at the method `jsDoc`. You can pass it a text or a descriptor. For
descriptors, it will use any comments on the Protobuf element, and add a helpful `@generated from ...` annotation.
...
You can generate import statements via a combination of the `import` and `print` methods.

For example, let's import the `useEffect` hook from React:

```typescript
const useEffect = f.import("useEffect", "react");
```

This returns an `ImportSymbol` that you can print:
...
It's common to import from `protoc-gen-es` generated code in other plugins. To make it as easy as possible, the method
`importSchema` imports the schema for a descriptor, and the method `importShape` imports the type:
...
Importing with the methods of `GeneratedFile` has many advantages, such as:
