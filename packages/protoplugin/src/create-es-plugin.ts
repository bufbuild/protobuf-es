...
interface PluginInit {
  ...
  /**
   * A optional parsing function which can be used to customize parameter parsing of the plugin.
   */
  parseOption?: PluginOptionParseFn;

  /*
   * A function which will generate TypeScript files based on proto input.  This function will be invoked by the
   * plugin framework when the plugin runs.
   *
   * Note that this is required to be provided for plugin initialization.
   *
   * @param {Schema} schema - Schema describes the files and types that the plugin is requested to generate.
   * @param {Target} target - Defaults to "ts" for TypeScript generator functions.
   *
   */
  generateTs: (schema: Schema, target: "ts") => void;

  /*
   * A optional function which will generate JavaScript files based on proto input.  This function will
   * be invoked by the  plugin framework when the plugin runs.
   *
   * If this function is not provided, the plugin framework will then check if a transpile function is provided.
   * If so, it will be invoked to transpile JavaScript files.  If not, the plugin framework will transpile
   * the files itself.
   *
   * @param {Schema} schema - Schema describes the files and types that the plugin is requested to generate.
   * @param {Target} target - Defaults to "js" for TypeScript generator functions.
   *
   */
  generateJs?: (schema: Schema, target: "js") => void;

  /*
   * A optional function which will generate TypeScript declaration files based on proto input.  This function will
   * be invoked by the  plugin framework when the plugin runs.
   *
   * If this function is not provided, the plugin framework will then check if a transpile function is provided.
   * If so, it will be invoked to transpile declaration files.  If not, the plugin framework will transpile
   * the files itself.
   *
   * @param {Schema} schema - Schema describes the files and types that the plugin is requested to generate.
   * @param {Target} target - Defaults to "dts" for TypeScript generator functions.
   *
   */
   ...
   *
   * This funcion is meant to be used in place of either generateJs, generateDts, or both.  However, those functions
   * will take precedence.  This means that if generateJs, generateDts, and this transpile function are all
   * provided, this transpile function will be ignored.
   *
   * @param {FileInfo[]} fileInfo - An array of FileInfo objects representing files to transpile.
   * @param {boolean} transpileJs - If true, the transpilation process should generate JavaScript files.
   * @param {boolean} transpileDts - If true, the transpilation process should generate TypeScript declration files.
   * @return {FileInfo[]} - An array of FileInfo objects representing files generated by the transpilation process.
   */
  transpile?: (
    files: FileInfo[],
    transpileJs: boolean,
