...
{
  private readonly ds: LegacyDescriptorSet;
  ...
  constructor(descriptorSet?: LegacyDescriptorSet, replaceWkt = true) {
    this.ds = descriptorSet ?? new LegacyDescriptorSet();
    if (replaceWkt) {
...
function localMethodName(protoName: string): string {
...
type ResolvedField = ReturnType<
  Exclude<
    LegacyDescriptorSet["messages"][string],
    undefined
