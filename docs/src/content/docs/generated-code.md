...
New messages created with `create()` initialize map fields to empty objects. Protobuf-ES deliberately uses objects instead of ECMAScript `Map` because objects fit better with current JavaScript and TypeScript tooling.
...
This representation works especially well with TypeScript's narrowing rules. It requires `strictNullChecks`, which is enabled by TypeScript's `strict` option.
