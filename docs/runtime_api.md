...
const set = FileDescriptorSet.fromBinary(readFileSync("./set.binpb"));
...
const registry = createRegistry(MessageA, MessageB);
...
if (process.argv.length !== 4) {
  console.error(
    `Usage: ${process.argv[1]} path-to-file-descriptor-set message-type-name`,
  );
  process.exit(1);
}
