...
func (g wktTimestamp) matches(message *protoplugin.Message) bool {
	_, _, err := g.getFields(message)
	return err == nil
}

func (g wktTimestamp) getFields(message *protoplugin.Message) (seconds *protoplugin.Field, nanos *protoplugin.Field, err error) {
...
func (g wktTimestamp) genWktMethods(f *protoplugin.GeneratedFile, message *protoplugin.Message) {
...
func (g wktTimestamp) genWktStaticMethods(f *protoplugin.GeneratedFile, message *protoplugin.Message) {
	rt := message.File.RuntimeSymbols
	seconds, nanos, _ := g.getFields(message)
