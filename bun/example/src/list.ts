...
const file = Bun.file(Bun.argv[2] ?? "users.binpb");
if (!(await file.exists())) {
  console.error(`File ${file.name} not found. Create users first, with add.ts`);
  process.exit(1);
}
...
for await (const user of sizeDelimitedDecodeStream(UserSchema, file.stream())) {
